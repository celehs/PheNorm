findMagicNumber <- function(surrogate, log_note_count, n.boot = 10) {
  ...
  for (k in 1:n.boot) {
    ...
    for (j in 1:length(coefs)) {
      ...
      fit <- normalmixEM2comp2(score, lambda = 0.5,
                               mu = quantile(score, probs = c(1/3, 2/3)), sigsqrd = 1
                               )
      # the following approximates \int|Fn(x)-Fmix(x)|dx
      Fn <- ecdf(score) # empirical cdf
      Fmix <- function(x) {
        fit$lambda[1] * pnorm(x, fit$mu[1], fit$sigma)
        + fit$lambda[2] * pnorm(x, fit$mu[2], fit$sigma)
      }
      id.small <- which.min(fit$mu)
      id.large <- which.max(fit$mu)
      fit.range <- seq(fit$mu[id.small] - 5 * fit$sigma,
                       fit$mu[id.large] + 5 * fit$sigma,
                       length.out = 1000
                       )
